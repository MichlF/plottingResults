...
if platform.system() == "Windows":
    projectFolder = 'F:\Dropbox\Work\Data\Behavioral\Attention\StatisticalRegularities\Salience\Exp2'
else:
    projectFolder = '/Users/michlf/Dropbox/Work/Data/Behavioral/Attention/StatisticalRegularities/Salience/Exp2'
analysisFolder = projectFolder+'/figures/'
if not os.path.exists(analysisFolder):
    os.makedirs(analysisFolder)
dataFile = '/Beh/AnalysisSAL2.xlsx'
...
# Data selection: RT
dataTarLoc = pd.pivot_table(data[(data.cond_disPresent == 'absent') & (data['RT<200'] == 0) & (data.correct == 1)],
...
# Data selection: ER
dataTarLocER = pd.pivot_table(data[(data.cond_disPresent == 'absent') & (data['RT<200'] == 0)],
...
# Data selection: RT
dataDisLoc = pd.pivot_table(data[(data.cond_disPresent == 'present') & (data['RT<200'] == 0) & (data.correct == 1)],
...
dataDisLoc = pd.melt(
    ...
    value_name='responseTime')
# Data selection: ER
dataDisLocER = pd.pivot_table(data[(data.cond_disPresent == 'present') & (data['RT<200'] == 0)],
...
# Data selection: RT
dataDisLocGrad = pd.pivot_table(data[(data.cond_disPresent == 'present') & (data['RT<200'] == 0) & (data.correct == 1)],
...
dataDisLocGrad = pd.melt(
    ...
    value_name='responseTime')
# Data selection: ER
dataDisLocGradER = pd.pivot_table(data[(data.cond_disPresent == 'present') & (data['RT<200'] == 0)],
